...
-   What's different about the 1.15.x port of the custom window title handling?
    -   The way setting and getting the window title is different in 1.15 and 1.15.1 compared to 1.15.2. As those are the only differences that impact this mod, I check the MC version at runtime and handle it accordingly
    -   `SharedConstants.getVersion().getName()` to detect the Minecraft version at runtime, stored as a `String` in the variable `mcVersion`
    -   If `mcVersion` is 1.15.2, treat getting and setting the window title the same as in 1.16.x
    -   Otherwise, treat it like MC 1.14.x
